...
截至当前 `main` 集成版本，MVP 和 A-T 并行开发任务已经全部完成。功能实现状态如下：

| 模块 | 状态 | 已实现内容 |
|---|---|---|
| 用户登录与账户管理 | ✅ 已完成 | JWT 登录、退出、当前用户、管理员用户列表、创建、编辑、禁用 |
| 图书基础信息管理 | ✅ 已完成 | 图书分页、搜索、筛选、详情、新增、编辑、删除、标签写入 |
| 简化中图法分类管理 | ✅ 已完成 | 44 条简化中图法种子、分类树、分类 CRUD、系统分类保护 |
| 位置管理 | ✅ 已完成 | 房间 / 书架 / 层数 / 具体位置、完整路径生成、位置 CRUD、引用保护 |
| 图书录入 | ✅ 已完成 | 手动录入、ISBN / 书名检索候选、AI 辅助入库、批量导入 |
| 网络检索与信息补全 | ✅ 已完成 | Open Library、Google Books、结果归一化、缓存、候选导入 |
| Ollama 本地模型集成 | ✅ 已完成 | 模型列表、分类推荐、标签生成、简介整理、重复判断、自然语言解析 |
| 搜索功能 | ✅ 已完成 | 图书关键词搜索、分类/位置/状态/年份/收藏筛选、外部检索、AI 自然语言解析接口 |
| 借阅管理 | ✅ 已完成 | 借出、归还、当前借出、历史记录、重复借出保护、状态同步 |
| 阅读记录与笔记 | ✅ 已完成 | 阅读状态、进度、评分、阅读笔记 CRUD、owner/admin 权限 |
| 统计分析 | ✅ 已完成 | 总览、最近入库、当前借出、分类/位置分布、阅读状态、入库趋势 |
| 导入导出 | ✅ 已完成 | CSV / JSON / Excel 导入预览、字段映射、确认导入、导出 |
| 备份恢复 | ✅ 已完成 | Docker 数据卷、SQLite 在线备份脚本、uploads 备份、恢复文档 |
| 系统设置 | ✅ 已完成 | 前端系统设置页、Ollama 模型选择、本地设置持久化、备份设置占位 |
| 部署与运维 | ✅ 已完成 | Dockerfile、Docker Compose、Nginx、NAS 部署文档 |
| 测试与质量 | ✅ 已完成 | 后端回归测试、前端 Vitest、Playwright E2E 冒烟测试 |

### 2.2 已知后续增强

以下内容属于原方案中的扩展方向，当前版本已保留接口或页面入口，但不作为 MVP 阻塞项：

- SQLite FTS5 / 向量检索：当前已实现数据库关键词搜索、筛选、外部检索和 AI 自然语言解析接口，FTS5/向量检索可后续独立优化。
- 二维码标签、扫码盘点、封面墙、年度阅读报告：属于体验增强，当前未纳入 A-T 必交范围。
- 后端持久化系统设置接口：当前设置页使用 `localStorage`，页面已有明确提示；后续可新增 `/api/settings`。
- 真实数据库联通 E2E：当前 E2E 为 mock API 冒烟骨架，完整联通真实后端数据库可在 CI/CD 阶段增强。
- 登录限流、外部 URL 图片代理、上传类型白名单等安全加固：已在安全设计中列出，可在生产化阶段继续补强。
