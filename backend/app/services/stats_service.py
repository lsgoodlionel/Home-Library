...
def get_category_distribution(db: Session) -> list[DistributionItem]:
    rows = (
        db.query(Category.id, Category.code, Category.name, func.count(Book.id))
    ...
    items = [
        DistributionItem(id=category_id, code=code, name=name, count=int(count))
        for category_id, code, name, count in rows
    ]
